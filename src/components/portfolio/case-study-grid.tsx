...
export function CaseStudyGrid({ locale }: { locale: AppLocale }) {
  const content = getPortfolioContent(locale);

  return (
    <section
      aria-label={content.sections.cases.ariaLabel}
      id="cases"
      className="mx-auto max-w-7xl px-6 py-10 sm:px-8 lg:px-10 lg:py-14"
    >
      <div className="max-w-3xl">
        <p className="text-sm font-semibold uppercase tracking-[0.08em] text-slate-500">
          {content.sections.cases.eyebrow}
        </p>
        <h2 className="mt-3 text-3xl font-semibold text-slate-950 sm:text-4xl">
          {content.sections.cases.title}
        </h2>
        <p className="mt-4 text-base leading-8 text-slate-600 sm:text-lg">
          {content.sections.cases.body}
        </p>
      </div>

      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        {content.homeCases.map((study) => (
          <article
            key={study.id}
          ...
          >
            <Link className="block h-full" href={study.href}>
              ...
              <div className="px-6 py-6">
                ...
                <h3 className="mt-2 text-2xl font-semibold text-slate-950">{study.brand}</h3>
